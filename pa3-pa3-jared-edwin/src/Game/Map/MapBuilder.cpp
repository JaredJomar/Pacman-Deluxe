...
Map* MapBuilder::createMap(ofImage mapImage){
	...
	if(!cherrySpawned) {
		...
		cherrySpawned = true;
		mapInCreation->getEntityManager()->powerups.push_back(cherry);
	}
	if(!strawberrySpawned) {
		...
		mapInCreation->getEntityManager()->powerups.push_back(strawberry);
	}
	if(!randomFruitSpawned) {
		...
		randomFruitSpawned = true;
		mapInCreation->getEntityManager()->powerups.push_back(randomFruit);
	}
